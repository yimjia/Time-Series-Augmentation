...
p1(i) = plot(T{i,1}(Len,Dim_Plot(1)),T{i,1}(Len,Dim_Plot(2)),'k-','linewidth',3);
p2(i) = plot(T_Auto{i,1}(Len,Dim_Plot(1)),T_Auto{i,1}(Len,Dim_Plot(2)),'r-.','linewidth',3);

for q = 1:2
    rho_raw = corrcoef(T{i,1}(Len,Dim_Plot(q)),T_Auto{i,1}(Len,Dim_Plot(q)));
    rho(q,1) = rho_raw(1,2);
    clearvars rho_raw
end
